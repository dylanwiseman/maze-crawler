...
export default function MazeCrawler() {
  ...
  return (
    <div className="container" id="gradient">
      ...
      <p>It's getting late... you should probably find your way home...</p>
      ...
      <div className="environment">
        <div className="maze-container">
          {maze.map((row, index) => {
            return (
              <div className="box-container" id={`row-${index}`} key={index}>
                {row.map((box, index) => {
                  return (
                    <div
                      className={`box ${
                        box === 1 || box === 2 || box === 7 ? "one" : ""
                      } ${box === 9 ? "nine" : ""} 
                                         ${box === 2 ? "two" : ""}`}
                      key={index}
                      id={`box-${index}`}
          ...
          <button
            ...
            onClick={() => {
              document
                ...
                .forEach((box) => box?.classList.remove("path"));
              setMaze(mazeBuilder());
            }}
          >
            new cave
